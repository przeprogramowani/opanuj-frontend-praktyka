...
export class CharacterSearchService {
    private charactersSource$: Observable<Character[]> = new Observable<Character[]>();
    ...
    constructor(private http: HttpClient) {
        this.charactersSource$ = combineLatest([this.nameSubject, this.genderSubject]).pipe(
            switchMap(([name, gender]) => {
                if (name !== '' || gender !== '') {
                    return this.http
                        .get<{ results: Character[] }>(`${this.apiBaseUrl}?name=${name}&gender=${gender}`)
        ...
        this.characters$ = combineLatest([this.charactersSource$, this.sortOptionSubject]).pipe(
            map(([characters, sortOption]) => this.sortCharacters(characters, sortOption))
        );
    }

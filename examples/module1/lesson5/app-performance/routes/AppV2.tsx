...
function Comments() {
    ...
    useEffect(() => {
        axios.get<{ comments: Comment[] }>(commentsAPI).then(({ data: { comments } }) => {
    ...
    return (
        <>
            {comments.length > 0 && <h2 className="font-bold text-xl my-2">Comments</h2>}
            {comments.map(comment => (
                <div className="bg-violet-400 rounded-xl text-white p-4 mb-2 shadow" key={comment.id}>
            ...
            {comments.length > 0 && (
                <form onSubmit={e => storeNewComment(e)}>
                    ...
                    <button className="bg-violet-400 text-white p-2 rounded-lg mt-2">Submit</button>
                </form>
            )}
        </>
    );
...
function Authors() {
    ...
    useEffect(() => {
        axios.get<{ authors: Author[] }>(authorsAPI).then(({ data: { authors } }) => {
            setAuthors(authors);
        });
    }, []);
    ...
    return (
        <>
            {authors.length > 0 && <h2 className="font-bold text-xl mb-2">Authors</h2>}
...
function Articles() {
    ...
    useEffect(() => {
        axios.get<{ articles: Article[] }>(articlesAPI).then(({ data: { articles } }) => {
    ...
    return (
        <>
            {articles.length > 0 && <h2 className="font-bold text-xl mt-2">Articles</h2>}
            ...
            {authors.length > 0 && <p className="text-right mt-4">Created by {authors.length} authors</p>}
        </>
    );
}

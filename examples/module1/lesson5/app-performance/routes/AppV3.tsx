    ...
    return (
        <>
            {Array.from({ length: lines }).map((_, i) => (
                <div key={i} className={`animate-pulse h-${height} bg-gray-300 rounded-lg mb-2`} />
            ))}
        </>
    );
}
...
function CommentsForm({ comments, dispatch }: { comments: Comment[]; dispatch: React.Dispatch<any> }) {
    const { commentsAPI } = useLoaderData() as Bootstrap;
...
function Articles() {
    ...
    return (
        <>
            ...
            {authors && <p className="text-right mt-4">Created by {authors.length} authors</p>}
        </>
    );
}
            ...
            <div className="max-w-7xl mx-auto">
                <div className="md:grid md:grid-cols-4 gap-x-8">
                    <div className="col-span-1">
                        ...
                        <img className="rounded-xl shadow mb-2" src={bookshelf} alt="Bookshelf" />
                        <Comments />
                    </div>

...
import { MESSAGE_ERROR_INVALID_INPUT, MESSAGE_ERROR_INVALID_INT, MESSAGE_SUCCESS_VALID_INT } from './messages';
...
export function validate(input: string, validators: NumericValidationMethod[]): string {
    ...
    const isValidRangeInteger = validators.every(validateFn => validateFn(Number(input)));

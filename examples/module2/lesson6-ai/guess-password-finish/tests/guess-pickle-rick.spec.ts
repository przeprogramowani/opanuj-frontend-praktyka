...
test('(Copilot) User can guess the password using hints', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="hint-text"]')).toHaveText('Ogórek i Rick połączeni w jedno');

    await page.locator('text=Pokaż podpowiedź').click();
    await expect(page.locator('[data-testid="hint-text"]')).toHaveText('Hasło to dwa słowa, drugie to imię');
...
test('(GPT-4) should guide the user through incorrect guesses, hints, and finally correct guess', async ({ page }) => {
    await page.goto('http://localhost:3000');

    await page.getByPlaceholder('Wpisz hasło...').fill('wrong password');
    await page.getByRole('button', { name: 'Zgadnij' }).click();

    await expect(page.getByTestId('guess-form').locator('.text-red-500')).toHaveText(
        'Niepoprawne hasło. Spróbuj ponownie lub skorzystaj z podpowiedzi.'
    );

    await page.getByRole('button', { name: 'Pokaż podpowiedź' }).click();

    await expect(page.getByTestId('hint-text')).toHaveText('Ogórek i Rick połączeni w jedno');

    await page.getByRole('button', { name: 'Pokaż podpowiedź' }).click();

    await expect(page.getByTestId('hint-text')).toHaveText('Hasło to dwa słowa, drugie to imię');

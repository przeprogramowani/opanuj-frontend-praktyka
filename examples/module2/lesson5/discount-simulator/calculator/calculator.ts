    ...
    calculateDiscount(purchase: Purchase): CalculationResult {
        ...
        for (const product of purchase.products) {
            ...
            if (this.discountConfig.categoryDiscounts[product.category]) {
                totalDiscount += product.price * this.discountConfig.categoryDiscounts[product.category];
            }
        }
        ...
        if (purchase.customer.loyaltyStatus !== 'None') {
            totalDiscount += subtotal * (this.discountConfig.loyaltyDiscounts[purchase.customer.loyaltyStatus] || 0);
        }
        ...
        totalDiscount = Math.min(totalDiscount, subtotal * this.discountConfig.maxDiscount);
